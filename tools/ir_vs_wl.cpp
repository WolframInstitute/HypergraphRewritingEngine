...
// over a full evolution the non-Full/WL route costs 68,004,245 instructions against Full/IR's
// 59,028,301 (callgrind, board #48), because the engine pays the ALLOCATION DISCIPLINE around
// the call and not only the call -- 9.74% of the non-Full run is __memset_avx2 zeroing WL
// scratch (board #43). Regenerate both with `tools/dev/evidence.sh`, which is where these came
// from; they drift as the engine changes and the gap between them is the claim, not the pair.
//
...
int main() {
    ...
    printf("NOTE: per-CALL cost of the algorithm, not the cost the engine pays. Over a full\n"
           "evolution non-Full/WL measured 68,004,245 instructions against Full/IR's 59,028,301\n"
