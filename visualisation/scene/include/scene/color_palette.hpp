...
constexpr math::vec4 VERTEX_SPHERE = {0.59f, 0.85f, 0.96f, 1.0f};  // Cyan

// Hyperedge lines connecting vertices
constexpr math::vec4 HYPEREDGE_LINE = {0.36f, 0.8f, 0.99f, 1.0f};  // Yellow

// Arrowheads on directed hyperedges
constexpr math::vec4 HYPEREDGE_ARROW = {0.36f, 0.8f, 0.99f, 1.0f};  // Orange-yellow

// Translucent bubble around hyperedges (for multi-vertex edges)
constexpr math::vec4 HYPEREDGE_BUBBLE = {0.36f, 0.8f, 0.99f, 0.05f};  // Magenta (translucent)

// Self-loop edges (edges connecting a vertex to itself)
constexpr math::vec4 SELF_LOOP = {0.36f, 0.8f, 0.99f, 1.0f};  // Orange
...
constexpr math::vec4 STATE_CUBE_NORMAL = {0.8f, 0.8f, 0.8f, 0.5f};

// Initial state cube wireframe (same as normal - transparent white)
constexpr math::vec4 STATE_CUBE_INITIAL = STATE_CUBE_NORMAL;

// Canonical state cube wireframe (same as normal - transparent white)
constexpr math::vec4 STATE_CUBE_CANONICAL = STATE_CUBE_NORMAL;

// State cube face alpha (translucent white fill, lower opacity ~15%)
constexpr float STATE_CUBE_FACE_ALPHA = 0.015f;
...
constexpr math::vec4 EVENT_EDGE = {0.9f, 0.9f, 0.9f, 0.75f};

// Causal edges (producer->consumer dependency) - orange/amber
constexpr math::vec4 CAUSAL_EDGE = {0.86f, 0.5f, 0.06f, 0.75f};

// Branchial edges (co-occurring events at same generation) - purple
constexpr math::vec4 BRANCHIAL_EDGE = {0.91f, 0.23f, 0.68f, 0.75f};
