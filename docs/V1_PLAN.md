...
| **#20** de-header | **CLOSED** `4614f18` | The floor is measured from BOTH sides. Cold bodies moved: `rule_analysis.hpp` 275 -> 163, `pattern.hpp` 492 -> 435, `signature.hpp` 300 -> 241. The include CLOSURE was the real lever -- `types.hpp` 757 -> 154 ms by dropping `<sstream>`, `<random>`, `<stdexcept>`, `<algorithm>`. Then the REMAINING 61 one-to-three-line member bodies of `hypergraph.hpp` were moved as an experiment and measured: **+0.21% instructions** (16,996,459 -> 17,032,941 under callgrind) and **0.00 s of compile time** (0.85-0.89 s before and after, on a TU including only that header). It costs runtime and buys nothing, because a header's cost is its include closure and not its bodies. Reverted; the numbers are in `tools/dev/deheader_shape.py` so it is not retried. What remains in headers is there because moving it was MEASURED not to pay | `python3 tools/dev/deheader_shape.py` |
