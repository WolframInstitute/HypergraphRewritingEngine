...
| P4.4 | #12 session model + `Result` — evolve returns a resumable handle, not a one-shot. | ENGINE HALF DONE `9c96d41`: `evolve_more(steps)` resumes from the frontier; N-1 then +1 equals N in one call on every corpus workload, on states, events, causal pairs, branchial, the state-hash multiset and the causal relation as a SET. The FFI half (four verbs, memoized artifact graph, splitting `run_rewriting_core`) stays open and is blocked on the transport allowing one in-flight job | **PARTIAL** |
...
| P4.6 | #82 residual: the COUNTS route through `observable_*`, but `CausalGraph`/`BranchialGraph` STRUCTURE is built from materialised raw-event records. | **MEASURED AND PINNED** `e1264bf`: on a 3-step chain under Automatic, NumEvents (reconstruction) 24 against 25 causal-graph vertices (materialised). The item's premise needed one correction — the vertices are ALREADY keyed by `effective_event_id`; the residual is WHICH event set is scanned. Closing it needs a vertex payload for an identity with no `Event` record, which the reconstruction deliberately does not materialise | **OPEN, pinned** |
