...
| **S1** | One IR: delete `IRCanonicalizer`, make the core escalate | IR is the only `hgcommon` core with a live second implementation — 1,771 lines of one algorithm, already divergent in generator discipline. De-headering two bodies doubles the work and bakes the divergence in. | **PART DONE** `535279b` `4036937` `86cae18` `4313f3c` `e6b4b4c` — exhaustion reported and retried, both host orbit sites on one body, core now emits canonical form and vertex labelling. **The deletion is NOT mechanical**: a thin adapter builds, passes the equivalence probe, and fails 18 tests including worker-count independence. The probe compares hash and form only, so it is blind to the class/rank/orbit outputs the divergence lives in. Gate those first. #112 |
| **S3** | One quotient reconstruction (P6.2, P6.2a rewritten) | The other duplicated rule: seven `qc_*` names implemented on both sides. Both copies have already drifted, neither drift caught by a gate. | pending, #41 |
| **S4** | Fix the duplication audit's two blind spots | It reported ONE cross-area duplicate (`main`) and missed both real ones — it compares names, so same-algorithm-different-name is invisible, and it compares QUALIFIED names, so host/device pairs in different namespaces are too. | pending, #113 |
| **S5** | Decide intra-state IR parallelism on device | `k_ir_canon_range` is one thread per state, so the GPU cannot accelerate a single hard canonicalisation — exactly the high-symmetry case where quotienting pays. Measure the distribution before building anything. | pending, #114 |
| **S6** | Re-read the incrementalisation probes | Their recorded verdict is what justified dropping incremental WL and pointing the lever at IR. Read what they measured before repeating it. | pending, #115 |
