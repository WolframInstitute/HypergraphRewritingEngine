...
| **#20** de-header | **IN PROGRESS** | Build-time lever exhausted with a per-header reason; `types.hpp` 757 → 170 ms and 765 → 503 lines; engine closure −142 ms; `run_rewriting_core` 1664 → 1129 over four one-way cuts. **CLOSED 2026-08-12.** Namespace convention landed (`9b62666` + `78285a6`): one `hg::` root, `-DHG_NAMESPACE` overridable, `nm` proves nine global names became one. Done-line set and MET (`4614f18`), recorded in `tools/dev/deheader_shape.py` so it cannot rot: de-headering is finished when every movable body large enough to matter is already in a translation unit or is on a hot path. Measured 404 movable bodies / 2100 lines, median 3 lines, 291 of 404 under six; exactly four exceed 40 lines and three are hot (the signature enumeration and its recursive helper, reached per pattern edge per state via `SignatureIndex::for_each_candidate`, and `MatchRecord::hash`). The signal that the floor moved is a NEW body over 40 lines that is not hot | all five, per commit |
...
| P7.2 | #24 paper + reproducible measurements. | GATE: *every number in it regenerable by one command* — **MET**. **The MACHINE is done** (`f73f557`): `tools/dev/evidence.sh` regenerates every headline number in one command — corpus exactness, static analysis, reconstruction vs full capture over 80 configurations, the quotient determinism rate, cost matrix, the callgrind route comparison, CODEMAP drift, the option surface, and both device suites. It REPORTS rather than asserts (the gates assert), uses no wall clock anywhere, and a leg that cannot run says SKIPPED with the guard that failed rather than being omitted. It earned its keep on the first run: the cited route figures had drifted ~7% and nothing would have said so (`ee69c66`). **The PAPER now carries measured tables and no placeholder.** `tools/dev/paper_tables.py` and `tools/dev/scaling_sweep.py` write `paper/tables/*.tex`, each with the commit, machine and tool in its own provenance line, and `main.tex` inputs them: T1 exactness/memory (17 workloads, all EXACT), T2 against the Wolfram authority (820x at depth 3 to 43902x at depth 7, state counts agreeing at every depth), T3 allocations against work, T6 quotient vs full capture, T7 GPU vs CPU with the crossover, T8 strong scaling to 32 threads WITH efficiency and a second problem size, T9 runtime-toggle ablation, T10 event-identity conventions, T11 device saturation. All nine `\todo` markers are closed, including three that asserted something false about the paper's own contents and a GPU section describing a Weisfeiler-Leman kernel the device does not run. **T2's basis is SETTLED:** `reference/authority_properties.wls` enumerates every property `MultiwaySystem` offers and each one is graph-valued, so no non-graph basis exists to time it against; the comparison is therefore the native engine against the Wolfram implementation as it ships, stated as such, and the claim carried is the ratio's GROWTH with depth (5,507x at four to 45,713x at seven). **T12/T13 close the scaling evidence** (`d4e1e60`): two rule shapes swept 1-32 threads at serial points of 5.7 s and 92.6 s, so the useful width is 8-16 workers and not the machine's 24 cores, and `getrusage` on the same runs shows the ceiling is per-worker memory acquisition -- user time 4.94 -> 10.28 s across a 32x worker increase while system time goes 0.27 -> 55.53 s tracking RSS 673 MB -> 8.1 GB. T8's caption no longer names simultaneous multithreading as the cause, which its own numbers do not support | **DONE** |
...
**NOT blockers, and marked so they stop reading as such:** #14 is CLOSED for v1.0 with an
on-demand knob as residue. The four this paragraph deferred have since been ANSWERED rather than
deferred, each by a measurement, and the answers are recorded so nobody re-opens them on a hunch:

- **#58** LANDED (`921a7ad`): `MatchesPerStateRule` keeps k per (state, rule), chosen at the drain
  by seeded rank, so the kept SET is identical across worker counts. Its first gate run found that
  the cap kept nothing whenever forwarding was off, because storing a state's own matches was
  gated on forwarding; recording and propagating are now separate concerns (`c4341c5`).
- **#30** CLOSED by measurement, and two of its three items do NOT survive it. Key-only sets landed
  earlier (causal closure, −28.5% arena). Chunked list nodes are NOT shipped: a chunk gains
  elements after being linked, which breaks the reader's termination rule, and the exact fix needs
  per-node reported-masks on the match/causal completeness surface — for arena bytes, not time.
  False sharing on the per-state `MatchJoin` measured 16.99 → 16.68 ms at depth 6 and 753 → 813 ms
  at depth 7: the direction REVERSES between workloads, so it is under this box's drift.
- **#77** REFUTED (`eb26d42`), and the diagnosis this row cites was wrong. The window partition was
  BUILT — per-node insertion positions on the prepend-only list, a per-child pull watermark,
  positions carried through both discovery sites and both pull re-propagations — and it moves the
  allocation count 8487 → 8455, 0.4%. Claim-loss is 0–1 copies throughout, so every allocation is
  the unique cover for a distinct (state, match) pair and there is nothing doubly covered to remove.
- **#24** is no longer deferred; see P7.2. **Needing Richard, not me:** #109 (reproduces on his laptop) and #116
