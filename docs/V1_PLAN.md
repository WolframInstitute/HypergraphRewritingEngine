...
| **21 of 22 shipped reference pages document symbols that do not exist** | `ls paclet/Documentation/English/ReferencePages/Symbols/` against `grep PackageExport paclet/Kernel/HypergraphRewriting.wl` | `PackageExport["HGEvolve"]` at line 5 is the ONLY export and the file has exactly one `::usage`. The other 21 pages — `EdgeId`, `HGTorus`, `HGSphere`, `HGToGraph`, `HGHausdorffAnalysis`, `HGBranchAlignmentBatch` and 15 more — name symbols with **zero** mentions anywhere in `paclet/Kernel/`, and all 21 are inside the assembled `.paclet`. `Tutorials/QuantumAnalysisExamples.wl` calls two of them. Same class as `fc6d24a`: the visualisation split removed the functions and both manifests outlived them. Disposition is Richard's — delete here, or the visualisation paclet carries them. Board #108 |
| **A generated tutorial notebook is 535 MB** | `ls -l "paclet/Documentation/Source/generated/Tutorials/Getting Started with Hypergraph Rewriting.nb"` | 535,665,769 bytes / 8,473,455 lines, 86% of the paclet tree. The embedded graph annotates EVERY `DirectedEdge` with its endpoints' complete `InputStateEdges` and `OutputStateEdges`, so the payload grows as events × state size and is duplicated again in the `Placed[...]` tooltip. Consumer is `HypergraphRewriting.wl:402-406`. It does not reach users — the archive ships `Documentation/English` and staging rejects `Documentation/Source` — but `DocumentationBuild` evaluates every example cell, so this is what that gate must chew through. Board #107 |
