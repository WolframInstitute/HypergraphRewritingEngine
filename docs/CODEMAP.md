...
- **`ancestry.hpp`** -- `AncestryCandidates`: a state's edges by ancestry, for candidate generation. Every edge of a state was produced by one event on its chain of parent events or belongs to the chain's root, so `for_each_edge_at` / `for_each_edge_containing_all` / `for_each_edge_compatible` walk that chain (each event's produced edges) and finish at the root's own vertex index (`State::root_index`, built once at creation). Nothing is maintained per edge on the write side.
