...
- **`quotient_route.hpp`** -- `quotient_route_requested(explore_canonical_states_only, positional, event_keys)`: whether a run asks for the quotient reconstruction. The host engine (`parallel_evolution.cpp`) and the FFI, which issues the `QuotientNeedsFull` warning for both devices when states are not Full, call it.
