...
- **`quotient_multiplicity_core.hpp`** -- RAW COUNTS FROM CLASS MULTIPLICITIES, one body for host and device, run in place of the replay when a request reads the raw events and branchial pairs only as counts (`RecordSet::raw_counts_only`, causal off). `m(class, depth)` accumulates the raw states a class stands for; `qm_pass` moves each match's unpassed mass on by compare-and-swap on `consumed_j(depth)` and counts `delta` events and `delta * b_j` branchial pairs; `qm_credit` adds mass and queues the point once; `qm_drain` runs queued points shallowest first (`qm_heap_push`/`qm_heap_pop`). Counts saturate at `QM_SATURATED` (2^63 - 1) with `QM_SATURATED_MESSAGE`. A Ctx supplies storage only: `Hypergraph::QmCtx` on the host, `DeviceQmCtx` on the device.
- **`quotient_route.hpp`** -- `quotient_route_requested(explore_canonical_states_only, positional, event_keys)`: whether a run asks for the quotient reconstruction. The host engine (`parallel_evolution.cpp`), the device (`evolve.cu`) and the FFI, which issues the `QuotientNeedsFull` warning for both devices when states are not Full, call it.
...
- **`pool_core.hpp`** -- THE CLAIM RULE OF A TAGGED FREE-LIST (Treiber stack, 16-bit tag over a 48-bit node ref), one body. `pool_core_push`/`pool_core_pop` over an Ops supplying the storage half (`head_load` ACQUIRE, `head_cas` ACQ_REL, `link_load`/`link_store` RELAXED). The popper's link read is SPECULATIVE -- a rival may win the node and rewrite its link before the loser's CAS fails -- so the tag makes the stale value harmless and the link accessors' atomicity makes the read defined; with a plain link access the algorithm still "works" on x86 and is a data race in the memory model. Driven by the arena's block pool (`hypergraph/src/arena.cpp` `BlockPoolOps`) and checked exhaustively by `verification/genmc/block_pool_exactly_once.cpp`, whose `-DCALIBRATE_PLAIN_LINK` arm reproduces the race.
...
- **`match.cu`** -- device `match_state_rule` + kernel `k_match_batch`. The JOIN is `hgcommon/join_core.hpp`; this file supplies `MatchJoinCtx` (CSR-slice / pivot-inverted / signature-bucket candidate enumeration) and the block-striped depth-0 parallelism. `emit_admit` applies the per-(state, rule) cap, the transition draw and the sampling spine as matches are emitted. Host `schedule_lhs_edges`/`make_device_rule`/`run_match_kernel*`
...
- **`event_identity.hpp`** -- event identity for the persistent kernel: `event_keys_need_ranks`, `run_needs_edge_ranks`, `run_needs_exact_hash`, `edge_rank_in_state_device`, `stamp_event_signature` (computes the signature AND applies it through a signature -> EventId map, so two applications with the same identity are one event)
...
- **`paclet_support.cpp`** -- the bodies behind the support headers: `session.hpp` (`EngineHolder`'s virtual destructor, so one TU anchors the vtable; `SessionSlot`'s open/engine/invalidate/close/require), `cpu_engine_holder.hpp`, `delivery_cursor.hpp`, `graph_marshal.hpp`'s free functions and `state_statistics.hpp`. ONE file rather than one per header, because five targets name their sources explicitly and each new file costs five list edits
...
- **`state_statistics.hpp`** -- "StepStatistics" for both engines: `state_invariants` (vertex and edge counts, arities, slot-degree sequence, 2-section edges, incidence-graph components, cycle ranks, and the diameter, mean distance and vertex fraction of the largest incidence component), `summarise` (a weighted population's moments, median and histogram), `events_from_multiplicities` and `step_statistics`, which summarises each step from one state per class weighted by the class's multiplicity. The definitions are the multiway-statistics probes'; `reference/verify_state_statistics.wls` checks them against a raw run
