...
- **`concurrent_id_set.hpp`** -- `ConcurrentIdSet<Key, EMPTY_KEY>`: lock-free, append-only, open-addressed SET, one atomic key per slot and no value word, so half the footprint of the `ConcurrentMap<K, uint8_t>` a set-shaped use would otherwise pay. Templated on the key type because the 64-bit claim sets are what it exists for. `for_each` skips a key a NEWER table already carries: resize COPIES and leaves the old table reachable, so a key surviving r resizes occupies r+1 slots and an unguarded walk emits it r+1 times
