...
- **`index.cpp`** -- the index MAINTENANCE side: `SignatureIndex::add_edge`, `InvertedVertexIndex::add_edge` (one entry per (vertex, edge), not per occurrence) with its counts, and `PatternMatchingIndex`. The query side is templated on the visitor and stays in the header.
