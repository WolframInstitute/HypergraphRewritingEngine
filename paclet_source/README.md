...
the physics analyses moved to the companion `hypergraph_viz` project.
...
- States are stored in arenas -- bump-allocated, reclaimed in bulk, no per-object ownership
...
- **C++ Core**: High-performance hypergraph rewriting engine
  ...
  - Canonicalization by an in-house McKay-style individualization-refinement canonicalizer
  - Lock-free concurrent hash maps

- **Interfaces**: an isolated `hg_evolve` process speaking WXF over stdio (the shipping path), and a LibraryLink DLL
