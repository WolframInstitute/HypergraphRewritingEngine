...
# report "No errors were detected". A line `// Args: <flags>` in the first two passes the flags
# to the checker. A checker without the patch aborts on the allocation and promotion
# reproducers with an internal check or reports a non-allocated access, does not finish
# dependence_dag_paths within the ten minutes this script gives it, reports no race on the
# cas_fail_* reproducers, fails memmove_overlap_tail's assertion and reports no error on
# copy_longer_than_unroll.
set -uo pipefail
...
for src in "$HERE"/*.cpp; do
    ...
    read -r -a args <<<"$(sed -n '1,2s|^// Args: ||p' "$src")"
