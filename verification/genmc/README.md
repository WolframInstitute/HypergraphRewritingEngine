...
| `--unroll=N` (a flag, carried in each engine harness's `GENMC-ARGS`; the caller's value overrides it) | every loop bounded to N iterations PER ENTRY; a thread that exceeds the bound is KILLED there | Unbounded, exploration of the composed engine ends in `INTERNAL FAILURE: getScheduler().inErrorReplay()` at `handleThreadKill`. | An execution whose threads were killed still counts as COMPLETE with no error, so the bound can hide everything past the kill. Each rung therefore carries a calibration arm, `-DHG_HARNESS_CALIBRATE_END=1`, an assertion that fails at the rung's end: a bound reaches the end when the checker reports it. Measured 2026-08-29: `engine_construct` and `engine_rule` reach their ends at `--unroll=16384` (kills at 64, 256, 1024 and 4096 before the capacity knobs below); at `--unroll=2` the main thread is killed inside construction after 1,237 events, before a worker exists. |
| `job_system.hpp` — `HG_JOB_QUEUE_CAPACITY` (4096) and `HG_JOB_INJECTOR_CAPACITY` (32768) | 16 and 64, from each engine harness's `GENMC-DEFINES`, next to `HG_SEGMENTED_ARRAY_MAX_SEGMENTS=8` and `HG_CONCURRENT_MAP_INITIAL_CAPACITY=16` | Each is a buffer filled slot by slot when the system is built, so it sets the loop bound construction needs. | Capacity is a size, not a protocol; the deque and injector run the same code at 16 slots. |
