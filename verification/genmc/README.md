...
| `EventLabel` (`CasReadLabel`, `ReadLabel::setRf`) + `Execution.cpp`: a compare-exchange's read takes the success ordering when the value it reads equals the expected value and the failure ordering otherwise, set again at every rf change (creation, candidate rfs, revisits) | The read label carried the success ordering only, so a failed CAS synchronised as if it had succeeded: a failed acquire-CAS whose success ordering is release synchronised with nothing, and a failed CAS with a relaxed failure ordering (`seq_cst`/`relaxed`, `acq_rel`/`relaxed`, `acquire`/`relaxed`, all used by the engine) synchronised with the store it read. | The block another thread published through a release CAS, taken by this thread's failed acquire-CAS, was "non-allocated" when written to: `grab_block`'s `old_head->next = new_block`. A first fix raised a failed read's ordering after the fact and never lowered it: a revisit that made the same CAS succeed kept the acquire (and gave the write part it created acquire too), hiding a race. | `checker/failed_cas_acquire.cpp`, `checker/cas_fail_then_succeed_relaxed.cpp`, `checker/cas_fail_relaxed_no_sync.cpp` |
| `PromoteMemIntrinsicPass.cpp`: a runtime-length `memset`/`memcpy`, an opaque one, and every `memmove` become bounded loops at the operands' width with a byte tail; a `memmove` whose destination lies inside its source copies the tail and then the words, each from the top; a copy whose loop would reach the `--unroll` bound fails an assertion | The interpreter's `memset()`/`memcpy()` handlers are an unconditional error, so one runtime-length memset stopped the program. The first loop form copied a descending `memmove`'s words before its tail, and the word stores cover the tail's source bytes. A lowered loop is bounded like any loop, and a copy longer than the bound killed its thread with no error. | "Invalid call to memset()" inside the engine's first rewrite. The overlap and bound defects were found by review; the reproducers fail on the checker before their fixes (wrong byte 4, and "No errors" with the thread killed in the copy). | `checker/memmove_overlap_tail.cpp`, `checker/copy_longer_than_unroll.cpp` |
| `GenMCDriver::handleError`: the error label takes a copy of the message | The message was moved into the label and then moved again into the report, which printed no "Assertion violation" line. | Every failed `assert` printed the graph without its expression. | `checker/copy_longer_than_unroll.cpp` (its expected text is the message) |
...
| `PromoteMemIntrinsicPass` runs twice: typed operands before SROA, opaque ones after SROA and mem2reg; intrinsic lowering once, after the second | A memcpy whose operands were a call result and a load from a lambda's closure field was lowered to a loop at the intrinsic's alignment, so a 152-byte `MatchCore` copy became i32 stores and the program's i16 field read (`MatchRecord::num_edges()`) lay inside one: a mixed-size access the value resolution cannot answer, reported as an uninitialised read. Every calibration arm of the full-engine harness stopped there in its first execution. | `create<T>(t)` reached through a `[&]` capture; the report now lists the writes near the address and says when the read lies inside a wider write | a 25-line program of that shape (`/tmp/mixed/create2.cpp` in the session; reported at width 2 inside an i32 store, passes now); five harnesses keep their exhaustive counts (86, 10, 4,020, 528, 2,500) |
...
(`lli/runtime-include/c/lkmm.h`), which pass the same ordering for success and failure; clang
lowers `cmpxchg_release`'s release failure ordering to `monotonic`, so under the third fix a
failed `cmpxchg_release` reads with relaxed ordering; PR #58 is the `checkLiveness` row.
...
| `job_system.hpp` — `WorkerThread`, `spawn_workers` | workers spawned on `run_worker` with the `JobSystem` as the only argument: worker 0 with `__VERIFIER_thread_create`, each further worker with `__VERIFIER_thread_create_symmetric` to its predecessor, no memory access between the spawn calls; a worker claims its index from `spawn_claim_` on entry and the thread ids are stored after the last spawn | `std::thread` wraps the same call in a heap-allocated polymorphic state whose typeinfo derives from libstdc++'s `std::thread::_State` -- an external constant with no definition in the module, which `InitializeMemory` follows to null. The checker's symmetry reduction applies only to threads with the same function and argument spawned with no memory access in between. | The body and the join are identical; the workers are interchangeable, so which one claims which index is the one permutation the reduction removes: `engine_construct` is 442 executions with it and 884 with `--disable-sr`. |
...
| `--unroll=N` (a flag, carried in each engine harness's `GENMC-ARGS`; the caller's value overrides it) | every loop bounded to N iterations PER ENTRY; a thread that exceeds the bound is KILLED there | Unbounded, exploration of the composed engine ends in `INTERNAL FAILURE: getScheduler().inErrorReplay()` at `handleThreadKill`. | An execution whose threads were killed still counts as COMPLETE with no error, so the bound can hide everything past the kill. A memory intrinsic the checker lowers to a loop (`memmove`, runtime-length or opaque `memset`/`memcpy`) counts against the same bound; a copy of N or more elements fails an assertion ("a memory intrinsic lowered to a loop is longer than the --unroll bound") instead of killing the thread. Each rung therefore carries a calibration arm, `-DHG_HARNESS_CALIBRATE_END=1`, an assertion that fails at the rung's end: a bound reaches the end when the checker reports it. Measured 2026-08-29: with the sizes below, `engine_construct` and `engine_rule` reach their ends at `--unroll=1024` (events 3,851 and 9,083); at `--unroll=2` the main thread is killed inside construction after 1,237 events, before a worker exists. |
| every construction-time size: `HG_MAX_PATTERN_EDGES` (16), `HG_MAX_CACHED_SIGS` (64), `HG_KEY_SET_SHARDS` (64), `HG_SEGMENTED_ARRAY_MAX_SHIFT` (uncapped), `HG_ARENA_BLOCK_SIZE` (4096 under verification), `HG_QC_CANON_EVENT_SEEN_CAPACITY` (4096), `HG_MAX_ARENA_WORKERS` (256), `HG_JOB_QUEUE_CAPACITY` (4096), `HG_JOB_INJECTOR_CAPACITY` (32768), with `HG_SEGMENTED_ARRAY_MAX_SEGMENTS` and `HG_CONCURRENT_MAP_INITIAL_CAPACITY` | 4, 8, 4, 4, 512, 16, 8, 16, 64, 8, 16 from each engine harness's `GENMC-DEFINES` | The checker replays the program's prefix for every revisit, and the prefix is construction: each of these sizes is a buffer, table or array zero-filled one store per element when the object is built. Under the shipped values the engine object was 35 KB, the hypergraph 72 KB and a rule 18.5 KB (record layouts, 2026-08-29); under these it is 3,320, 8,104 and 835 bytes, and `engine_construct`'s end is reachable at `--unroll=1024` (event 3,851, where it was 54,611 at 16384). | A size is not a protocol: every structure runs the same code at the smaller capacity, and the harnesses' inputs fit them. |
| `affinity.hpp` — `affinity_backend()` | `None` when `HG_VERIFICATION` is defined | The Linux backend's `performance_cpus()` reads `/sys/devices/system/cpu` through `snprintf` and `fopen`, which the interpreter cannot execute. | `start()` takes the path it takes on macOS: no thread is bound. Placement is not a property any harness states. |
| `genmc_support.cpp` — `sched_yield()` | defined, returns 0 | `std::this_thread::yield()` in `start()`'s entry wait is this call, and the checker has no model of it. | A yield is a scheduling hint with no memory semantics; the checker explores every interleaving regardless. |
| the engine's instruments (claim ring, silent witnesses, drain-probe and index miss-witness strings, the placement trace's `getenv`) | absent: `HG_ENGINE_STATS=0`, which `run.sh` passes | They are storage of stats builds only (`hgcommon/core.hpp`); as members of every build they were an 8 MB fill at construction, `std::string` constructors and a `getenv` the interpreter cannot execute. | Their readers are HG_STAT sites and the determinism test, and none is a property a harness states. |
