...
void ParallelEvolutionEngine::configure_identity_and_quotient() {
    ...
    // Forwarding is off whenever a draw can fail. The per-state cap and the sampling spine both
    // decide at a state's drain over the state's own matches, and a forwarded match arrives after
    // the drain. With forwarding on, the cap on {{x,y},{x,z}} -> {{x,z},{x,w},{y,w},{z,w}} from
    // {{0,0},{0,0}}, k = 1, reached 300 states by depth 5 where it allows 6; and the spine chose
    // from the delta matches alone, a different set from the device's, which matches every
    // (state, rule) pair in full. With forwarding off, a state's own matching finds all of its
    ...
    const bool counts_only = rec.raw_counts_only && !rec.causal;
    const bool replay = rec.causal || rec.branchial || rec.raw_events;
    hg_->set_quotient_reconstruction(qc && (replay || rec.multiplicities));
