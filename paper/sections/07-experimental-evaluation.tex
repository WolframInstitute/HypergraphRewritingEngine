...
\subsection{Comparison with MultiwaySystem}
...
Table~\ref{tab:t7-gpu}, at 1, 2, 4, 8, 16, 24 and 32 workers; the host has sixteen
...
The cost appears in wall time at the worker that first uses a second L3 domain. On this part an L3
domain holds two cores. With one worker per physical core, wall time rises from two workers to
three on every large workload (\texttt{wpp} by $12.7\%$, \texttt{multirule} by $36.1\%$), and
placing the third worker on an SMT sibling in the first domain removes the rise. The engine
therefore fills each domain's two cores and their SMT siblings before using the next domain. This
placement is faster at every worker count measured, including sixteen (\texttt{wpp} $197$ to
$182$\,ms, \texttt{multirule} $34.7$ to $25.7$\,ms), although at sixteen it uses half as many
physical cores, and it was used for every table in this section.
...
\textbf{Host traffic.} Across every evolution traced, including twenty-four consecutive evolutions
with the reconstruction on, no copy, allocation or memset runs while the resident kernel runs.
...
side (depth six) it turns forwarding on, and forwarding on is $1.10\times$ slower than off. Both settings give
