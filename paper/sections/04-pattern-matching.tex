...
\Procedure{FindMatches}{$L$, $\hypergraph$}
    ...
    \State Enqueue $\textsc{ScanTask}(e_1)$
    \While{task queue non-empty}
        \State $\tau \gets \textsc{Dequeue}()$
        \If{$\tau$ is $\textsc{ScanTask}(e_1)$}
            \ForAll{$e \in \hypergraph$ with $\sigma(e)$ compatible with $\sigma(e_1)$}
                \State $\phi \gets$ binding of $e_1$ to $e$
                \State Enqueue $\textsc{ExpandTask}(2, \phi)$
            \EndFor
        \ElsIf{$\tau$ is $\textsc{ExpandTask}(i, \phi)$}
            \If{$i > k$}
                \State \textbf{yield} complete match $\phi$
            \Else
                \ForAll{$e \in \textsc{Candidates}(e_i, \phi)$ not in $\phi$}
                    \If{$e$ is consistent with $\phi$ on $e_i$}
                        \State Enqueue $\textsc{ExpandTask}(i{+}1, \phi \cup \{e_i \mapsto e\})$
                    \EndIf
                \EndFor
            \EndIf
        \EndIf
...
product, and $\textsc{Candidates}(e_i, \phi)$ is the incidence list of a vertex of $e_i$ already
bound in $\phi$ (Section~\ref{sec:vertex-index}). The host runs the join in two forms: a depth-first join that runs a whole search inside one task
