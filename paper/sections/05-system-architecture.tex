...
engine's case is simpler in two ways. Causal edges are only inserted, so an edge dropped as implied
never needs to be reconsidered. And they arrive in topological order, because an event's producers
...
descending order, while reachability from $p'$ requires $p' < x$, a contradiction. So deciding each
...
At large depth exhaustive evolution is too large to compute, and a caller may want only part of
